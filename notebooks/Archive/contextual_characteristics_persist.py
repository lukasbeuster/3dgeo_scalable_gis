...
def get_contextual_characteristics_for_buildings(primary, buildings, spatial_weights, client, serial=False):
    ...
    if serial:
        ...
        # Create a Dask client to manage computations
        client = client
        ...
        # Persist the delayed results
        persisted_results = dask.persist(*delayed_results)
        
        # Compute the persisted results and store them in a list
        results = dask.compute(*persisted_results)
